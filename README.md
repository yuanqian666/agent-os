...
#   自上而下 Agent 树（root 在顶，连线向下）：初始 root 直连 HAA；
#   繁殖后 HAA 动态挂载到携带其基因的族系子下（root 经子 agent 间接
#   连接 HAA，符合公链语义 HAA→族系子→Root）；已销毁节点灰显保留 15s
...
- **公理 2 公链 = 自下而上的物理权限专线**：每个 HAA（唯一单例叶子）向上由父子关系连到 Root 的唯一链条；基因与公链一一对应（gene→HAA 映射）
- **公理 3 族系 = 任务拆解的嵌套逻辑树**：每个 Agent 的子代与后代构成其族系，族系内再繁殖形成嵌套子族系；**族系与基因解耦**——族系是任务拆解的"业务部门"，不同族系可携带若干相同基因
